...
function BenchmarkTool() {
  ...
  // Refs for tooltip positioning and click outside detection
  ...
  const generateChart = (chartType) => {
    ...
    if (benchmarkResults.length > 0) {
      ...
      benchmarkResults.forEach((result) => {
        const matrixSize =
          parseInt(result.matrixSize.split("x")[0]) *
          parseInt(result.matrixSize.split("x")[1]);
      ...
      benchmarkResults.forEach((result, index) => {
        ...
        ctx.fillText(
          `${chartType === "execution" ? value + "ms" : "dist: " + value}`,
          x + 8,
          y + 8
        );
      });
      ...
      if (chartType === "execution" && benchmarkResults.length > 1) {
        ...
        const x1 = padding;
        const y1 =
          padding + height - ((intercept + slope * 0) / maxValue) * height;
        ...
        const y2 =
          padding +
          height -
          ((intercept + slope * maxMatrixSize) / maxValue) * height;
  ...
  const runBenchmark = (additionalCases = []) => {
    ...
    const processedCustomCases = additionalCases.map((customCase) => {
      ...
      const matrixSize = `${customCase.str1.length + 1}x${
        customCase.str2.length + 1
      }`;
      const cellCount =
        (customCase.str1.length + 1) * (customCase.str2.length + 1);

      // Simulate Levenshtein distance (simplified)
      let distance = 0;
      const minLength = Math.min(
        customCase.str1.length,
        customCase.str2.length
      );
      ...
      const executionTime = (
        baseTime +
        (Math.random() * variability - variability / 2)
      ).toFixed(1);
  ...
  const complexityInfo = {
    timeComplexity:
      "O(m×n) where m and n are the lengths of the two strings. The algorithm builds a matrix of size (m+1)×(n+1) and fills each cell exactly once.",
    spaceComplexity:
      "O(m×n) due to the dynamic programming matrix required to store intermediate results during computation.",
    optimizationNote:
      "For long strings, memory usage can be optimized to O(min(m,n)) by only storing the current and previous rows of the matrix, as each cell only depends on its neighbors.",
    practicalConsiderations:
      "Performance degrades quadratically with string length. Consider specialized algorithms for very long strings (e.g., DNA sequences) or approximate methods for fuzzy matching at scale.",
  };
    ...
    <div className="benchmark-container relative">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div className="flex items-center">
          ...
          <div className="relative ml-2">
            <button
              ...
              aria-label="Benchmark tool information"
            >
              <svg
            ...
            {infoTooltipVisible && (
              <div
                ref={tooltipRef}
                className="absolute z-10 w-72 md:w-80 right-0 sm:left-0 top-8 p-4 bg-white rounded-lg shadow-lg border border-slate-200 text-sm text-slate-700"
              >
                <div className="font-medium mb-2 text-teal-700">
                  Performance Analysis Tool
                </div>
                <p className="mb-2">
                  This tab evaluates how the Levenshtein algorithm performs with
                  different input sizes and helps understand its computational
                  efficiency.
                </p>
                <p className="mb-2">
                  Understanding algorithm performance is crucial for:
                </p>
                <ul className="list-disc pl-4 space-y-1">
                  <li>Optimizing applications for large datasets</li>
                  <li>Predicting resource requirements</li>
                  <li>Identifying performance bottlenecks</li>
                  <li>Making informed implementation choices</li>
                </ul>
        ...
        <div className="flex gap-3 w-full sm:w-auto">
          <button
            ...
            className="px-4 py-2 rounded-lg bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 transition-all duration-200 text-sm flex-1 sm:flex-none whitespace-nowrap"
          >
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-lg bg-gradient-to-r from-teal-400 to-emerald-500 text-white hover:from-teal-500 hover:to-emerald-600 disabled:opacity-70 transition-all duration-300 shadow-sm hover:shadow-md transform hover:translate-y-px flex-1 sm:flex-none whitespace-nowrap"
            disabled={isLoading}
          >
            {isLoading ? (
            ...
            {customTestCases.map((testCase, idx) => (
              ...
              >
                <div className="text-slate-700 truncate mr-2">
                  <span className="font-medium">{testCase.str1}</span> →{" "}
                  <span className="font-medium">{testCase.str2}</span>
                </div>
                <button
                  onClick={() => removeCustomTestCase(idx)}
                  className="text-rose-500 hover:text-rose-700 transition-colors flex-shrink-0"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-5 w-5"
      ...
      {isLoading ? (
        <div className="relative text-center py-16 bg-gradient-to-r from-slate-50 to-slate-100 rounded-xl overflow-hidden">
          ...
          <p className="text-slate-700 mt-4 font-light">
        ...
        <>
          {/* Chart Tabs */}
          <div className="mb-4 flex border-b border-slate-200 overflow-x-auto scrollbar-hide">
            <button
                      ...
                      <thead>
                        <tr>
                          ...
                          {benchmarkResults.slice(0, 3).map((result, idx) => (
                            <th
                              key={idx}
                              className="px-4 py-3 text-left text-xs font-semibold text-slate-700 bg-slate-50"
                            >
                              {result.description}
                            </th>
                          ))}
                      ...
                      <tbody>
                        {runHistory.slice(-3).map((run, runIdx) => (
                          <tr
                            key={run.id}
                            className={
                              runIdx % 2 === 0 ? "bg-white" : "bg-slate-50"
                            }
                          >
                            ...
                            <td className="px-4 py-3 text-sm font-medium text-slate-700">
                              {(
                                run.results.reduce(
                                  (acc, curr) =>
                                    acc + parseFloat(curr.executionTime),
                                  0
                                ) / run.results.length
                              ).toFixed(1)}{" "}
                              ms
          ...
          <div className="p-8 rounded-xl bg-white shadow-sm border border-slate-200 hover:shadow-md transition-all duration-300 relative overflow-hidden">
            ...
            <div className="relative">
              <h3 className="text-xl font-light mb-6 pb-3 border-b border-slate-200 text-slate-700">
                <span className="font-medium">Computational</span> Complexity
                Insights
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-6 rounded-lg bg-gradient-to-br from-white to-slate-50 border border-slate-200 shadow-sm hover:shadow-md transition-all duration-300 group">
                  <h4 className="flex items-center text-slate-700 font-medium mb-3">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-5 w-5 mr-2 text-teal-600 group-hover:text-teal-700 transition-colors duration-300"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                    >
                      <path
                ...
                <div className="p-6 rounded-lg bg-gradient-to-br from-white to-slate-50 border border-slate-200 shadow-sm hover:shadow-md transition-all duration-300 group">
                  <h4 className="flex items-center text-slate-700 font-medium mb-3">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-5 w-5 mr-2 text-emerald-600 group-hover:text-emerald-700 transition-colors duration-300"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                    >
                ...
                <div className="p-6 rounded-lg bg-gradient-to-br from-white to-slate-50 border border-slate-200 shadow-sm hover:shadow-md transition-all duration-300 group">
                  ...
                  <p className="text-slate-600">
                    {complexityInfo.optimizationNote}
                  </p>
                </div>
                ...
                <div className="p-6 rounded-lg bg-gradient-to-br from-white to-slate-50 border border-slate-200 shadow-sm hover:shadow-md transition-all duration-300 group">
                  ...
                  <p className="text-slate-600">
                    {complexityInfo.practicalConsiderations}
                  </p>
                </div>
              </div>
        ...
        <div className="relative text-center py-20 bg-gradient-to-r from-slate-50 to-slate-100 rounded-xl border border-slate-200 shadow-sm overflow-hidden group">
          ...
          <p className="mt-5 text-slate-600 max-w-md mx-auto">
            No performance data available yet. Click "Run Benchmark" to analyze
